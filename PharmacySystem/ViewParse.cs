...
namespace PharmacySystem
{
    ...
    internal static class ViewParse
    {
        ...
        public static int ComboInt(ComboBox combo, int fallback = 0) =>
            ...
            && int.TryParse(((item.Value ?? string.Empty).ToString() ?? string.Empty).Trim(), out int value)
                ? value
                : fallback;

        public static string ComboText(ComboBox combo, string fallback = "") =>
            combo?.SelectedItem is ComboBoxItem item ? item.Text : fallback;

        public static string ComboValueText(ComboBox combo, string fallback = "") =>
            combo?.SelectedItem is ComboBoxItem item ? ((item.Value ?? fallback).ToString() ?? fallback) : fallback;
    }
}
